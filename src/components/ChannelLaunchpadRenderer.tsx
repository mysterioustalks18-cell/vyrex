...
const ScoreRing = ({ score = 0, label }: { score?: number; label: string }) => {
  const displayScore = score <= 1 ? score * 100 : score <= 10 ? score * 10 : score;
  return (
    <div className="flex flex-col items-center">
      <Tooltip 
        content={
          displayScore >= 85 ? "Optimal high-tier performance potential" : 
          displayScore >= 75 ? "Strong market-ready performance" : 
      ...
      >
        <div style={{position:'relative', display:'inline-block'}} className="cursor-help">
          <svg width="120" height="120" viewBox="0 0 120 120">
            ...
            <motion.circle 
              cx="60" cy="60" r="52" 
              stroke={displayScore >= 85 ? '#22d3a0' : displayScore >= 75 ? '#00c8ff' : '#f59e0b'}
              strokeWidth="8" fill="none"
              strokeDasharray="326.7"
              initial={{ strokeDashoffset: 326.7 }}
              animate={{ strokeDashoffset: 326.7 * (1 - displayScore/100) }}
              transition={{ duration: 1.4, ease: "easeOut" }}
              strokeLinecap="round"
              transform="rotate(-90 60 60)"
              style={{filter:`drop-shadow(0 0 8px ${displayScore >= 85 ? '#22d3a0' : '#00c8ff'})`}}
            />
...
export const ChannelLaunchpadRenderer = ({ data, isExample }: { data: ChannelLaunchpadResult; isExample?: boolean }) => {
  ...
  return (
    <div className="space-y-16 pb-32">
      ...
      {/* SECTION 4: AUDIENCE INTELLIGENCE DEEP-DIVE */}
      <div className="space-y-8">
        <div className="font-mono text-[10px] text-vy-cyan font-bold uppercase tracking-[0.2em] flex items-center gap-2">
          <UserIcon className="w-4 h-4" /> AUDIENCE ARCHITECTURE & AVATAR PROFILE
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          {/* Demographics Matrix */}
          <ResultCard index={9} className="md:col-span-4 p-6 bg-[#0d1525] border-l-2 border-vy-cyan" showShimmer={isExample}>
            <div className="space-y-6">
              <div>
                <div className="font-mono text-[9px] text-vy-muted uppercase tracking-widest mb-3">DEMOGRAPHIC MATRIX</div>
                <div className="flex items-end gap-3">
                  <div className="text-4xl font-display font-black text-white italic leading-none">{data.audienceProfile.primaryAge}</div>
                  <div className="font-mono text-[10px] text-vy-cyan font-bold uppercase mb-1">AGE RANGE</div>
                </div>
              </div>
              
              <div className="pt-6 border-t border-white/5">
                <div className="font-mono text-[8px] text-vy-muted uppercase tracking-widest mb-3">GENDER DISTRIBUTION</div>
                <div className="text-xl font-display font-bold text-white uppercase italic">{data.audienceProfile.gender}</div>
                <div className="mt-3 h-1.5 w-full bg-white/5 rounded-full overflow-hidden flex">
                  <div className="h-full bg-vy-cyan" style={{ width: data.audienceProfile.gender.toLowerCase().includes('male') && data.audienceProfile.gender.toLowerCase().includes('female') ? '50%' : '100%' }} />
                </div>
              </div>

              <div className="pt-6 border-t border-white/5">
                <div className="font-mono text-[8px] text-vy-muted uppercase tracking-widest mb-3">PRIMARY TRIGGER</div>
                <p className="text-xs font-bold text-vy-cyan italic leading-relaxed">
                  "{data.audienceProfile.whatMakesThemSubscribe}"
                </p>
              </div>
            </div>
          </ResultCard>

          {/* Psychographic & Habit Layers */}
          <div className="md:col-span-8 grid grid-cols-1 sm:grid-cols-2 gap-6">
            <ResultCard index={10} className="p-6 bg-[#0d1525]" showShimmer={isExample}>
              <CardHeader icon={Target} title="PSYCHOGRAPHIC PROFILE" />
              <div className="space-y-3">
                {data.audienceProfile.psychographics.map((trait, i) => (
                  <div key={i} className="flex gap-3 items-center p-2.5 bg-white/5 rounded-xl border border-white/5">
                    <div className="w-1.5 h-1.5 rounded-full bg-vy-violet shadow-[0_0_8px_rgba(139,92,246,0.5)]" />
                    <span className="text-[11px] font-bold text-white uppercase tracking-tight">{trait}</span>
                  </div>
                ))}
              </div>
            </ResultCard>

            <ResultCard index={11} className="p-6 bg-[#0d1525]" showShimmer={isExample}>
              <CardHeader icon={AlertTriangle} title="CORE PAIN POINTS" />
              <div className="space-y-3">
                {data.audienceProfile.painPoints.map((point, i) => (
                  <div key={i} className="flex gap-3 items-start p-2.5 bg-vy-accent/5 rounded-xl border border-vy-accent/10">
                    <AlertTriangle className="w-3.5 h-3.5 text-vy-accent shrink-0 mt-0.5" />
                    <span className="text-[11px] font-bold text-[#ff8c42] uppercase tracking-tight leading-tight">{point}</span>
                  </div>
                ))}
              </div>
            </ResultCard>

            <ResultCard index={12} className="p-6 md:col-span-2 bg-gradient-to-r from-[#0d1525] to-[#161c2e]" showShimmer={isExample}>
              <div className="flex flex-col md:flex-row gap-8 items-center">
                <div className="w-16 h-16 rounded-2xl bg-vy-violet/20 flex items-center justify-center shrink-0 border border-vy-violet/30">
                  <Clock className="w-8 h-8 text-vy-violet" />
                </div>
                <div className="space-y-2">
                  <div className="font-mono text-[9px] text-vy-muted uppercase tracking-[0.3em]">BEHAVIORAL WATCHING HABITS</div>
                  <p className="text-sm font-display font-medium text-white italic leading-relaxed">
                    {data.audienceProfile.watchingHabits}
                  </p>
                </div>
              </div>
            </ResultCard>
      ...
      {/* SECTION 8.5: SHORTS VIABILITY */}
      {data.shortsViability && (
        <div className="space-y-8">
          <div className="font-mono text-[10px] text-vy-cyan font-bold uppercase tracking-[0.2em]">⚡ SHORTS VIABILITY ANALYSIS</div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <ResultCard index={13} className="p-6 md:p-8 bg-gradient-to-br from-vy-cyan/5 to-[#0d1525] md:col-span-1 flex flex-col items-center justify-center text-center" showShimmer={isExample}>
               <ScoreRing score={data.shortsViability.viabilityScore / 10} label="SHORTS POTENTIAL" />
               <div className="mt-6">
                  <div className="font-mono text-[9px] text-vy-cyan font-black uppercase tracking-widest mb-2">DISCOVERY POTENTIAL</div>
                  <p className="text-sm font-display font-black text-white uppercase italic leading-tight">{data.shortsViability.discoveryPotential}</p>
               </div>
            </ResultCard>

            <ResultCard index={14} className="p-6 md:p-8 bg-[#0d1525] md:col-span-2 space-y-6" showShimmer={isExample}>
               <div className="space-y-4">
                  <div className="font-mono text-[10px] text-vy-cyan font-bold uppercase tracking-widest flex items-center gap-2">
                    <TrendingUp className="w-4 h-4" /> AUDIENCE ENGAGEMENT PATTERNS
                  </div>
                  <p className="text-sm font-medium text-[#c8dce8] leading-relaxed italic">
                    "{data.shortsViability.engagementPatterns}"
                  </p>
               </div>
               
               <div className="pt-6 border-t border-white/5">
                  <div className="font-mono text-[10px] text-vy-accent font-black uppercase tracking-widest mb-4">SHORTS VIRALITY BLUEPRINT</div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {data.shortsViability.viralityBestPractices.map((practice, i) => (
                      <div key={i} className="flex gap-3 items-start p-3 bg-white/5 rounded-xl border border-white/5">
                        <Check className="w-4 h-4 text-vy-green shrink-0 mt-0.5" />
                        <span className="text-[11px] font-bold text-white uppercase tracking-tight leading-tight">{practice}</span>
                      </div>
                    ))}
                  </div>
               </div>
