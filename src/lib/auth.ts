...
const createUserRecord = async (user: User) => {
  ...
  try {
    ...
    handleFirestoreError(err, 'write', `users/${user.uid}`, auth);
  }
};
...
export const signUpWithEmail = async (email: string, password: string, name: string) => {
  try {
    ...
    try {
      ...
      handleFirestoreError(err, 'create', `users/${result.user.uid}`, auth);
    }
...
export const getUserData = async (uid: string) => {
  try {
    ...
    handleFirestoreError(err, 'get', `users/${uid}`, auth);
    return null;
  }
};
...
export const deductCredits = async (uid: string, amount: number) => {
  try {
    ...
    try {
      ...
      handleFirestoreError(err, 'update', `users/${uid}`, auth);
    }
